...
int main(int argc, char** argv) {
    ...
    while (ch != KEY_SEND) {
        ...
        if (ch == KEY_DOWN) {
            if(mode == 0)
                RepNormal.SendKey(2,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_UP) {
            if(mode == 0)
                RepNormal.SendKey(1,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_LEFT) {
            if(mode == 0)
                RepNormal.SendKey(4,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_RIGHT) {
            if(mode == 0)
                RepNormal.SendKey(3,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_HOME) {
            if(mode == 0)
                RepNormal.SendKey(5,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_NPAGE) {
            if(mode == 0)
                RepNormal.SendKey(7,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_PPAGE) {
            if(mode == 0)
                RepNormal.SendKey(6,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_END) {
            if(mode == 0)
                RepNormal.SendKey(8,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == Enter_Key){
            if(mode == 0)
                RepNormal.SendKey(9,CurrentLine,x,y,RepScreen,RepFile,Rep);
            continue;
        }
        if (ch == KEY_BACKSPACE) {
            if(mode == 0)
